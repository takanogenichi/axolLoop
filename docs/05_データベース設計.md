...
> **注記**: 本設計は Amazon Aurora MySQL 8.0+ を前提とする。アクセス制御（旧仕様で想定していた PostgreSQL の Row Level Security）は、NestJS の Guards・Decorators・Interceptors で実装する。
...
| カラム名 | データ型 | 制約 | 説明 |
|----------|----------|------|------|
| `user_id` | `VARCHAR(255)` | PK | ユーザー ID（UUID。Node.js `crypto.randomUUID()` で生成） |
...
| カラム名 | データ型 | 制約 | 説明 |
|----------|----------|------|------|
| `department_id` | `VARCHAR(255)` | PK | 部署 ID（UUID。Node.js `crypto.randomUUID()` で生成） |
...
**アクセス制御（NestJS Guards で実装）:**

- `transcript_text` へのアクセスは、当該セッションの `manager_id`・`subordinate_id` 本人、および `role_type = 'hr'` のユーザーに限定する（`SessionAnalysisGuard.viewTranscript()` で制御）
- `ai_summary` と `action_items` は `manager_id`・`subordinate_id` 本人に限定する（`SessionAnalysisGuard.viewSummary()` で制御）
...
- 監査ログは追記のみ（UPDATE / DELETE 不可）。アプリケーション層（Prisma Middleware および NestJS Interceptors）で UPDATE / DELETE を禁止する
...
3. **プライバシー保護（NestJS Guards + Interceptors）**
   - `session_analysis` の `transcript_text`（生データ）は、当該セッションの上司・部下本人、および `role_type = 'hr'` のユーザーに限定する。アクセス制御は NestJS の Guards・Decorators で実装する。
